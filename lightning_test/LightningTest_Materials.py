...
diffuse_loc = glGetUniformLocation(light_shader, "light.diffuse")
specular_loc = glGetUniformLocation(light_shader, "light.specular")
...
while not glfw.window_should_close(window):
    ...
    glUniform3f(light_position_loc, *lamp_pos_vector)
    glUniform3f(view_pos_loc, *camera.position)

    lightColor = pyrr.Vector3((np.sin(glfw.get_time()*2.0), np.sin(glfw.get_time()*0.7), np.sin(glfw.get_time()*1.3)))
    diffuseColor = lightColor * pyrr.Vector3((0.5, 0.5, 0.5))
    ambientColor = diffuseColor * pyrr.Vector3((0.2, 0.2, 0.2))
    glUniform3f(light_ambient_loc, *ambientColor)
    glUniform3f(diffuse_loc, *diffuseColor)
    glUniform3f(specular_loc, 1.0, 1.0, 1.0)

    glUniform3f(material_ambient_loc, 1.0, 0.5, 0.31)
    glUniform3f(material_diffuse_loc, 1.0, 0.5, 0.31)
    glUniform3f(material_specular_loc, 1.5, 1.5, 1.5)
