...
cube_vertices = [-0.5, -0.5,  0.5, 0.0, 0.0,    # XYZUV
             0.5, -0.5,  0.5, 1.0, 0.0,
             0.5,  0.5,  0.5, 1.0, 1.0,
            -0.5,  0.5,  0.5, 0.0, 1.0,
...
quad_vertices = [-0.5, -0.5, 0, 0.0, 0.0,   # XYZUV
                  0.5, -0.5, 0, 1.0, 0.0,
                  0.5,  0.5, 0, 1.0, 1.0,
                 -0.5,  0.5, 0, 0.0, 1.0]
...
triangle_vertices = [-0.5, -0.5, 0, 1, 0, 0,    # XYZGGB
                      0.5, -0.5, 0, 0, 1, 0,
                      0.0,  0.5, 0, 0, 0, 1]
...
while not glfw.window_should_close(window):
    ...
    model = pyrr.matrix44.multiply(rx, quad_pos)
    glBindVertexArray(quad_VAO)
    glBindTexture(GL_TEXTURE_2D, texture[1])
    glUniformMatrix4fv(model_loc, 1, GL_FALSE, model)
    glDrawElements(GL_TRIANGLES, len(quad_indices), GL_UNSIGNED_INT, None)

    model = pyrr.matrix44.multiply(ry, triangle_pos)
    glBindVertexArray(triangle_VAO)
