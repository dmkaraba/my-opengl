...
class Car:
    def __init__(self):
        ...
        self.headVector = pyrr.Vector3((np.sin(self.headAngle), 0, -np.cos(self.headAngle)))
    ...
    def forward(self):
        ...
        if self.SPEED > 0.4:
            self.SPEED = 0.4
        self.position = self.position + self.headVector * self.SPEED
    ...
    def turn_left(self):
        self.headAngle = self.headAngle - self.TURN_DELTA
        self.headVector = pyrr.vector3.normalize(pyrr.Vector3((np.sin(self.headAngle), 0, -np.cos(self.headAngle))))
...
while not glfw.window_should_close(window):
    ...
    print(car.SPEED)
    # set Camera
    camera.cameraPosition = car.position - pyrr.vector3.normalize(car.headVector)*2 + pyrr.Vector3((0, 1, 0))
