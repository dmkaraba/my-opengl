...
def key_callback(window, key, scancode, action, mods):
    ...
    if key == glfw.KEY_S:
        car.backward()
    # if key == glfw.KEY_D:
    #     self.cameraPosition = self.cameraPosition + self.MOVEMENT_SPEED * self.strafeDirection
    # if key == glfw.KEY_A:
    #     self.cameraPosition = self.cameraPosition - self.MOVEMENT_SPEED * self.strafeDirection
    # if key == glfw.KEY_R:
    #     self.cameraPosition = self.cameraPosition + self.MOVEMENT_SPEED * self.upVector
    # if key == glfw.KEY_F:
    #     self.cameraPosition = self.cameraPosition - self.MOVEMENT_SPEED * self.upVector
...
class Car:
    def __init__(self):
        ...
        self.headDirection = pyrr.Vector3((0, 0, -1))
...
while not glfw.window_should_close(window):
    ...
    pos = pyrr.matrix44.create_from_translation(car.position)
