...
class MyObjLoader:
    @classmethod
    def parse_v_vt_vn(cls, line):
        return map(float, line.split()[1:])

    @classmethod
    def parse_f(cls, line):
        values = line.split()[1:]
        vl = [i.split('/') for i in values]
        indices = [int(i[0]) for i in vl]
        all_indices = [int(val) for sublist in vl for val in sublist]
        return indices, all_indices

    @classmethod
    def load_model(cls, file_path):
        vert_coords = list()        # will contain all the vertex coordinates
        tex_coords = list()         # will contain all the texture coordinates
        norm_coords = list()        # will contain all the vertex normals
        indices_list = list()       # will contain the indices for indexed drawing
        all_indices_list = list()   # will contain all the vertex, texture and normal indices
        with open(file_path, 'r') as f:
            line = f.readline()
            while line:
                t = line.split()[0]
                if t == 'v':
                    vert_coords.extend(cls.parse_v_vt_vn(line))
                elif t == 'vt':
                    tex_coords.extend(cls.parse_v_vt_vn(line))
                elif t == 'vn':
                    norm_coords.extend(cls.parse_v_vt_vn(line))
                elif t == 'f':
                    indices, all_indices = cls.parse_f(line)
                    indices_list.extend([i-1 for i in indices])
                    all_indices_list.extend([i-1 for i in all_indices])
                line = f.readline()
        buffer = cls.create_sorted_vertex_buffer(all_indices_list, vert_coords, tex_coords, norm_coords)
        return np.array(indices_list, dtype='uint32'), np.array(buffer, dtype='float32')

    @classmethod    # sorted vertex buffer for use with glDrawArrays function
    def create_sorted_vertex_buffer(cls, indices_data, vertices, textures, normals):
        buffer = list()
        for i, ind in enumerate(indices_data):
            if i % 3 == 0:  # sort the vertex coordinates
                start = ind * 3
